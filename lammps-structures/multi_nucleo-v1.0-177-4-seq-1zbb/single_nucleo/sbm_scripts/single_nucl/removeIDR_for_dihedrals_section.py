####################################################################################
# This script will help read in the extracted [ dihedrals ] section the smog.top file and
...
def removeIDR_for_dihedrals_section(inputFile, outputFile):

    dihedrals_interactions = np.loadtxt('dihedrals.dat', comments=';')
    ...
    for i in my_lt_range(0, np.shape(dihedrals_interactions)[0], 1):
        bead_0 = int(dihedrals_interactions[i][0])
        bead_1 = int(dihedrals_interactions[i][1])
        bead_2 = int(dihedrals_interactions[i][2])
        bead_3 = int(dihedrals_interactions[i][3])
        a = bead_0 % num_CA
        b = bead_1 % num_CA
        c = bead_2 % num_CA
        d = bead_3 % num_CA

        if (a not in histonetailSegment) and (b not in histonetailSegment) and (c not in histonetailSegment) and (d not in histonetailSegment) and ((bead_0 - 1)//num_CA == (bead_1 - 1)//num_CA) and ((bead_1 - 1)//num_CA == (bead_2 - 1)//num_CA) and ((bead_2 - 1)//num_CA == (bead_3 - 1)//num_CA):
            outfile.write(str(int(dihedrals_interactions[i][0])) + "\t" + str(int(dihedrals_interactions[i][1])) + "\t" + str(int(dihedrals_interactions[i][2])) + "\t" + str(int(dihedrals_interactions[i][3])) + "\t" + str(dihedrals_interactions[i][4]) + "\t" + str(dihedrals_interactions[i][5]) + "\t" + str(dihedrals_interactions[i][6]) + "\n")
