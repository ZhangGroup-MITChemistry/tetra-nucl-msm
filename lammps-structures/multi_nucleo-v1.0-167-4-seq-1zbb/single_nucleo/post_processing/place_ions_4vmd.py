...
def place_ions(num_of_ions, outputFile, sta_coord_x, sta_coord_y, sta_coord_z, grid_size, chainID_sta, resid_sta, num_existing_atoms, atom_type, charge):
    ...
    count = 0
    for idx_x in my_lt_range(0, 19, 1):
        for idx_y  in my_lt_range(0, 19, 1):
            for idx_z in my_lt_range(0, 19, 1):
                ...
                if count <= num_of_ions:
                    ...
                    outfile.write(str(num_existing_atoms) + " " + str(chainID_sta) + " " + str(atom_type) + " " + str(charge) + " " + str(coord_x) + " " + str(coord_y) + " " + str(coord_z) + "\n")
